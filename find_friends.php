...
union
select add_profile.*,'ADD FRIEND'  as friend_status from add_profile where reg_id not in(select req_id from friend_request where user_id='$id') and reg_id not in (select user_id from friend_request where req_id='$id') and reg_id!=$id");
